        ...
        <div class="col-lg-6 col-6 text-left">
            <form action="">
                <div class="input-group">
                    <input type="text" class="form-control" placeholder="Nhập sản phẩm cần tìm kiếm">
    ...
    <div class="row border-top px-xl-5">
        <div class="col-lg-3 d-none d-lg-block">
            ...
            <nav
                ...
                id="navbar-vertical" style="width: calc(100% - 30px); z-index: 99;">
                <div class="navbar-nav w-100 overflow-hidden" style="height: 410px">
                    @foreach ($categories as $item)
                        <div class="{{$item->getChildren->count() ? 'nav-item dropdown' : ''}}">
                            <a href="{{ route('client.category', ['slug'=>$item->slug]) }}" class="nav-link"
                               data-toggle="{{$item->getChildren->count() ? 'dropdown' : ''}}">{{$item->name}} <i
                                    class="{{$item->getChildren->count() ? 'fa fa-angle-down float-right mt-1' : ''}}"></i></a>
                            <div
                                class="{{$item->getChildren->count() ? 'dropdown-menu position-absolute bg-secondary border-0 rounded-0 w-100 m-0' : ''}}">
                                @foreach ($item->getChildren as $cate)
                                    <a href="{{ route('client.category', ['slug'=>$cate->slug]) }}"
                        ...
                        @foreach ($menus as $menu)
                            <div class="{{$menu->getChildren->count() ? 'nav-item dropdown' : 'nav-item'}}">
                                <a href="{{ url('/', [$menu->slug]) }}"
                                   data-toggle="{{$menu->getChildren->count() ? 'dropdown' : ''}}"
                                   class="{{$menu->getChildren->count() ? 'dropdown-toggle active' : ''}} nav-link">{{$menu->name}}</a>
                                @if ($menu->getChildren->count())
                                    <div class="dropdown-menu rounded-0 m-0">
                                        @foreach ($menu->getChildren as $sub_menu)
                                            <a href="{{ url('/', [$sub_menu->slug]) }}"
                                               class="dropdown-item">{{$sub_menu->name}}</a>
                                        @endforeach
                                    </div>
                                @endif
                            </div>
                    ...
                    <div class="navbar-nav ml-auto py-0">
                        @auth
                            <form action="{{ url('client/logout') }}" method="post">
                                {{ csrf_field() }}
                                <button type="submit" class="nav-item btn btn-primary">Đăng xuất</button>
                            </form>
                        @else
